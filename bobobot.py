...
def new_member_validation(update, context, tries):
    ...
    try:
        handlers_remover(dispatcher, 'mention_handler', user_id=candidate.id, group=2)
        handlers_remover(dispatcher, 'not_mention_handler', user_id=candidate.id, group=2)
    except Exception as e:
        logging.info('new_member_validation:'+ repr(e))
    ...
    if inviter and inviter.invites > 0:
        ...
        dispatcher.add_handler(yes_handler, group=1)
        dispatcher.add_handler(no_handler, group=1)
    else:
...
def positive_validation(update, context, additional_context):
    ...
    try:
        unset_timer(update, context)
        handlers_remover(dispatcher, 'yes_handler', user_id=inviter.tg_id, group=1)
        handlers_remover(dispatcher, 'no_handler', user_id=inviter.tg_id, group=1)
    ...
    dispatcher.add_handler(about_handler, group=0)
    dispatcher.add_handler(not_about_handler, group=0)

def add_new_member(update, context):
    print('add_new_member')
    chat_id, candidate, inviter = context
    handlers_remover(dispatcher, 'about_handler', user_id=candidate.id, group=0)
    handlers_remover(dispatcher, 'not_about_handler', user_id=candidate.id, group=0)
...
def alarm(context):
    ...
    try:
        ...
        try:
            handlers_remover(dispatcher, 'mention_handler', user_id=target.id, group=2)
            handlers_remover(dispatcher, 'not_mention_handler', user_id=target.id, group=2)
        except KeyError:
            handlers_remover(dispatcher, 'yes_handler', user_id=inviter.tg_id, group=1)
            handlers_remover(dispatcher, 'no_handler', user_id=inviter.tg_id, group=1)
        until_date = int(time.time())+31
...
if __name__ == '__main__':
    ...
    dispatcher.add_handler(MessageHandler(filters=(Filters.regex(r'nnn')), callback=new_member_start), group=3)
    dispatcher.add_handler(MessageHandler(filters=(~Filters.regex(r'nnn')), callback=add_msg_to_db), group=3)
